...
### Node.js action warning

Treat runtime deprecation warnings from third-party actions as non-blocking unless the affected step fails. Do not set insecure compatibility flags merely to hide a warning. Upgrade actions only through a separately reviewed workflow change.
...
The original setup was validated by workflow run `29549050132` with install, test, typecheck, lint, and build passing. This run is historical evidence, not proof of current runner availability. Recheck the current commit SHA and terminal workflow result before declaring a new change green.
