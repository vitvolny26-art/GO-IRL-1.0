...
const weatherIcon = (kind: "condition" | "rain" | "wind", x: number, y: number) => {
  if (kind === "rain") {
    return `<g data-weather-icon="rain" fill="none" stroke="#f5f7f8" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"><path d="M${x + 5} ${y + 15}h24a9 9 0 0 0-2-18 12 12 0 0 0-22 5 7 7 0 0 0 0 13z"/><path d="M${x + 10} ${y + 22}l-3 7M${x + 20} ${y + 22}l-3 7M${x + 30} ${y + 22}l-3 7"/></g>`;
  }
  if (kind === "wind") {
    return `<g data-weather-icon="wind" fill="none" stroke="#f5f7f8" stroke-width="4" stroke-linecap="round"><path d="M${x} ${y + 7}h24c7 0 7-9 1-9-4 0-5 3-5 5M${x} ${y + 17}h33c8 0 8 10 1 10-5 0-6-4-6-6M${x} ${y + 27}h17"/></g>`;
  }
  return `<g data-weather-icon="condition" fill="none" stroke="#f5f7f8" stroke-width="4" stroke-linecap="round" stroke-linejoin="round"><circle cx="${x + 12}" cy="${y + 8}" r="7"/><path d="M${x + 12} ${y - 4}v-4M${x + 12} ${y + 20}v4M${x} ${y + 8}h-4M${x + 24} ${y + 8}h4M${x + 3} ${y - 1}l-4-4M${x + 21} ${y - 1}l4-4"/><path d="M${x + 10} ${y + 25}h25a8 8 0 0 0-2-16 11 11 0 0 0-20 5 7 7 0 0 0-3 11z" fill="#111518"/></g>`;
};
...
const weatherBlock = (input: TelegramEventCardInput) => {
  ...
  return `<g font-family="DejaVu Sans, sans-serif" font-size="30" font-weight="800" fill="#f5f7f8">
    ${weatherIcon("condition", 76, 486)}<text x="128" y="520">${Math.round(weather.temperature)}°C</text>
    ${weatherIcon("rain", 76, 548)}<text x="128" y="582">${Math.round(weather.rain)}%</text>
    ${weatherIcon("wind", 76, 610)}<text x="128" y="644">${Math.round(weather.wind)} km/h</text>
  </g>`;
};
...
const buildShareCardSvg = (input: TelegramEventCardInput) => {
  ...
  <defs><linearGradient id="readability" x1="0" y1="0" x2="0" y2="1"><stop stop-color="#030506" stop-opacity="0.86"/><stop offset="0.46" stop-color="#030506" stop-opacity="0.28"/><stop offset="1" stop-color="#030506" stop-opacity="0.9"/></linearGradient></defs>
  ...
  <g data-share-participants="two-row" font-family="DejaVu Sans, sans-serif" font-weight="900"><g fill="none" stroke="#c9ff3d" stroke-width="5" stroke-linecap="round"><circle cx="930" cy="126" r="13"/><path d="M906 171c2-19 11-29 24-29s22 10 24 29"/></g><text x="930" y="220" text-anchor="middle" fill="#f7f8f9" font-size="32">${xml(participantCount)}</text></g>
  ${weatherBlock(input)}
  <g data-share-footer="two-row">
    <line x1="242" y1="714" x2="242" y2="846" stroke="#f5f7f8" stroke-opacity="0.2" stroke-width="2"/><line x1="510" y1="714" x2="510" y2="846" stroke="#f5f7f8" stroke-opacity="0.2" stroke-width="2"/><line x1="750" y1="714" x2="750" y2="846" stroke="#f5f7f8" stroke-opacity="0.2" stroke-width="2"/>
    <rect data-organizer-avatar-slot="soft-square" x="78" y="716" width="128" height="128" rx="16" fill="#111518" fill-opacity="0.42" stroke="#c9ff3d" stroke-opacity="0.58" stroke-width="3"/><text x="142" y="793" text-anchor="middle" fill="#f7f8f9" font-size="42" font-weight="900" font-family="DejaVu Sans, sans-serif">${xml(organizerInitial)}</text>
    ${metricIcon("calendar", 358, 735)}<text x="376" y="826" text-anchor="middle" fill="#f7f8f9" font-size="27" font-weight="900" font-family="DejaVu Sans, sans-serif">${xml(dateTime)}</text>
    ${metricIcon("ticket", 611, 735)}<text x="630" y="826" text-anchor="middle" fill="#f7f8f9" font-size="27" font-weight="900" font-family="DejaVu Sans, sans-serif">${xml(price)}</text>
    ${metricIcon("pin", 856, 732)}<text fill="#f7f8f9" font-size="24" font-weight="900" font-family="DejaVu Sans, sans-serif">${textLines(wrap(place, 20, 1), 874, 826, 30, "middle")}</text>
  </g>
  </svg>`;
};
