...
export async function handleMessengerTestInvitation(
  request: Request,
  ...
  const configuredToken = readEnv("MESSENGER_TEST_TRIGGER_TOKEN");
  ...
  try {
    event = await dependencies.getEventSummary(eventId);
  } catch {
  ...
  try {
    await dependencies.sendInvitation(recipientId, event);
  } catch {
