...
const coachCardCopy: Record<Language, { needed: string; requested: string; confirmed: string }> = {
  ru: { needed: "Пригласить тренера", requested: "Тренер запрошен", confirmed: "Есть тренер" },
  uk: { needed: "Запросити тренера", requested: "Тренера запитано", confirmed: "Є тренер" },
  cs: { needed: "Pozvat trenéra", requested: "Trenér vyžádán", confirmed: "Trenér potvrzen" },
  en: { needed: "Invite coach", requested: "Coach requested", confirmed: "Coach confirmed" },
};

const organizerEditCopy: Record<Language, string> = {
  ru: "Править",
  uk: "Редагувати",
  cs: "Upravit",
  en: "Edit",
...
export function SportActivityCard({ activity, language, onOpen, onJoin }: SportCardProps) {
  ...
  const cardRightLabel = isOrganizer ? organizerEditCopy[language] : joined || waiting ? t.leave : pending ? t.cancelRequest : action;
