...
type ShareCardRow = {
  ...
  card_status: BeautyShareCard["status"];
...
const dataUrlToBlob = async (value: string) => {
  ...
  if (!response.ok) throw new Error("beauty_share_data_url_decode_failed");
...
const signedAssetUrl = async (path: string | null) => {
  if (!path) return "";
  const { data, error } = await supabase.storage.from(assetBucket).createSignedUrl(path, 60 * 60);
  if (error) throw error;
  return data.signedUrl;
};

const generatedPublicUrl = (path: string | null) => path
  ? supabase.storage.from(generatedBucket).getPublicUrl(path).data.publicUrl
  : "";
...
export const loadRemoteBeautyShareCard = async (workspace: BeautyWorkspace): Promise<BeautyWorkspace> => {
  ...
  if (result.error) {
    if (result.error.code === "PGRST202") return workspace;
    throw result.error;
  }
  ...
  return {
    ...
    shareCard: {
      ...workspace.shareCard,
      enabled: row.card_status !== "deleted",
      backgroundImageDataUrl,
      logoImageDataUrl,
      backgroundPositionY: row.background_position_y,
      serviceIds: Array.isArray(row.service_ids) ? row.service_ids.slice(0, 3) : workspace.shareCard.serviceIds,
      status: row.card_status,
      generatedImageDataUrl: generatedPublicUrl(row.generated_object_path),
...
export const saveRemoteBeautyShareCard = async (workspace: BeautyWorkspace) => {
  ...
  const result = await supabase.rpc("save_my_beauty_share_card", {
    ...
    p_background_object_path: backgroundObjectPath,
    p_logo_object_path: logoObjectPath,
    p_generated_object_path: generatedObjectPath,
    p_background_position_y: card.backgroundPositionY,
    p_service_ids: card.serviceIds.slice(0, 3),
    p_source_fingerprint: card.sourceFingerprint,
    p_error_message: card.errorMessage,
    p_generated_at: card.generatedAt || null,
