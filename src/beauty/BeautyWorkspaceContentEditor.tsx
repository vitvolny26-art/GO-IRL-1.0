...
const copy = {
  ru: {
    title: "Страница мастера и прайс",
    hint: "Пустые необязательные блоки не показываются клиентам.",
    profile: "Профиль",
    portfolio: "Портфолио",
    prices: "Прайс",
    ...
    city: "Город",
    publicLocation: "Район для клиентов",
    contact: "Контакт мастера",
    exactAddress: "Точный адрес",
    description: "О мастере",
    instagram: "Instagram",
    experience: "Опыт",
    ...
    bookingNotes: "Важное перед записью",
    portfolioHint: "Добавьте прямые ссылки на фотографии. Пустое портфолио не отображается.",
    addWork: "Добавить работу",
    imageUrl: "Ссылка на изображение",
    imageAlt: "Описание фото",
    open: "Открыть",
    remove: "Удалить",
    pricesHint: "Нужна минимум одна активная услуга с названием, длительностью и ценой.",
  ...
  uk: {
    title: "Сторінка майстра і прайс",
    hint: "Порожні необов’язкові блоки не показуються клієнтам.",
    profile: "Профіль",
    portfolio: "Портфоліо",
    prices: "Прайс",
    required: "Обов’язково",
    optional: "Необов’язково",
    publicName: "Публічне ім’я",
    city: "Місто",
    publicLocation: "Район для клієнтів",
    contact: "Контакт майстра",
    exactAddress: "Точна адреса",
    description: "Про майстра",
    instagram: "Instagram",
    experience: "Досвід",
    ...
    bookingNotes: "Важливе перед записом",
    portfolioHint: "Додайте прямі посилання на фотографії. Порожнє портфоліо не відображається.",
    addWork: "Додати роботу",
    imageUrl: "Посилання на зображення",
    imageAlt: "Опис фото",
    open: "Відкрити",
    remove: "Видалити",
    pricesHint: "Потрібна щонайменше одна активна послуга з назвою, тривалістю та ціною.",
    ...
    price: "Ціна, Kč",
    buffer: "Буфер, хв",
    cannotRemove: "Не можна видалити єдину послугу",
  },
  cs: {
    title: "Profil profesionála a ceník",
    hint: "Prázdné nepovinné sekce se klientům nezobrazí.",
    profile: "Profil",
    portfolio: "Portfolio",
    prices: "Ceník",
    required: "Povinné",
    optional: "Nepovinné",
    publicName: "Veřejné jméno",
    city: "Město",
    publicLocation: "Oblast pro klienty",
    contact: "Kontakt profesionála",
    exactAddress: "Přesná adresa",
    description: "O profesionálovi",
    instagram: "Instagram",
    experience: "Praxe",
    specialization: "Specializace",
    hygiene: "Hygiena a sterilizace",
    materials: "Materiály a značky",
    languages: "Jazyky komunikace",
    certificates: "Vzdělání a certifikáty",
    bookingNotes: "Důležité před rezervací",
    portfolioHint: "Přidejte přímé odkazy na fotografie. Prázdné portfolio se nezobrazí.",
    addWork: "Přidat práci",
    imageUrl: "Odkaz na obrázek",
    imageAlt: "Popis fotografie",
    open: "Otevřít",
    remove: "Odstranit",
    pricesHint: "Je nutná alespoň jedna aktivní služba s názvem, délkou a cenou.",
    ...
    price: "Cena, Kč",
    buffer: "Buffer, min",
    cannotRemove: "Jedinou službu nelze odstranit",
  },
  en: {
    title: "Professional page and price list",
    hint: "Empty optional sections are not shown to clients.",
    profile: "Profile",
    portfolio: "Portfolio",
    prices: "Price list",
    required: "Required",
    ...
    city: "City",
    publicLocation: "Client-facing area",
    contact: "Professional contact",
    exactAddress: "Exact address",
    description: "About the professional",
    instagram: "Instagram",
    ...
    portfolioHint: "Add direct image links. An empty portfolio stays hidden.",
    addWork: "Add work",
    imageUrl: "Image URL",
    imageAlt: "Image description",
    open: "Open",
    remove: "Remove",
    pricesHint: "At least one active service with a name, duration, and price is required.",
    ...
    price: "Price, CZK",
    buffer: "Buffer, min",
    cannotRemove: "The only service cannot be removed",
  },
...
export function BeautyWorkspaceContentEditor({
  workspace,
  language,
  onChange,
}: {
  ...
  const updateProfile = <K extends keyof BeautyWorkspace["profile"]>(key: K, value: BeautyWorkspace["profile"][K]) => {
    onChange({ ...workspace, profile: { ...workspace.profile, [key]: value } });
  };
  ...
  const addService = () => {
    ...
    updateServices([...workspace.services, {
      ...service,
      name: "",
      nameByLanguage: emptyBeautyLocalizedText(),
      durationMinutes: 60,
      priceCzk: 0,
      bufferMinutes: 0,
    }]);
  ...
  const updateService = (index: number, patch: Partial<BeautyService>) => {
    updateServices(workspace.services.map((service, itemIndex) => itemIndex === index ? { ...service, ...patch } : service));
  };

  const removeService = (index: number) => {
    if (workspace.services.length <= 1) return;
    updateServices(workspace.services.filter((_, itemIndex) => itemIndex !== index));
  };

  const updatePortfolio = (portfolio: BeautyWorkspace["portfolio"]) => onChange({
    ...workspace,
    portfolio: portfolio.map((item, index) => ({ ...item, sortOrder: index })),
  });

  return <section className="beauty-workspace-content-editor">
    <header>
      <div><h2>{text.title}</h2><p>{text.hint}</p></div>
      <span>{text.required}: 5 + 1</span>
    </header>

    <nav className="beauty-workspace-content-tabs" aria-label={text.title}>
      {(["profile", "portfolio", "prices"] as Tab[]).map((item) => <button type="button" key={item} className={tab === item ? "is-active" : ""} onClick={() => setTab(item)}>{text[item]}</button>)}
    </nav>

    <div className="beauty-workspace-language-tabs" aria-label="Content language">
      {beautyContentLanguages.map((item) => <button type="button" key={item} className={contentLanguage === item ? "is-active" : ""} onClick={() => setContentLanguage(item)}>{languageNames[item]}</button>)}
    </div>
    ...
    {tab === "portfolio" && <div className="beauty-workspace-portfolio-editor">
      <p>{text.portfolioHint}</p>
      {workspace.portfolio.map((item, index) => <article key={item.id}>
        <div className="beauty-workspace-portfolio-preview">{item.imageUrl ? <img src={item.imageUrl} alt="" /> : <ImagePlus />}</div>
        <div>
          <label><span>{text.imageUrl}</span><input type="url" inputMode="url" value={item.imageUrl} onChange={(event) => updatePortfolio(workspace.portfolio.map((work, itemIndex) => itemIndex === index ? { ...work, imageUrl: event.target.value } : work))} /></label>
          <label><span>{text.imageAlt} · {languageNames[contentLanguage]}</span><input value={item.altByLanguage[contentLanguage]} onChange={(event) => updatePortfolio(workspace.portfolio.map((work, itemIndex) => itemIndex === index ? { ...work, altByLanguage: { ...work.altByLanguage, [contentLanguage]: event.target.value } } : work))} /></label>
          <div className="beauty-workspace-row-actions">
            <button type="button" disabled={index === 0} onClick={() => updatePortfolio(move(workspace.portfolio, index, -1))}><ArrowUp /></button>
            <button type="button" disabled={index === workspace.portfolio.length - 1} onClick={() => updatePortfolio(move(workspace.portfolio, index, 1))}><ArrowDown /></button>
            ...
            <button type="button" className="danger" onClick={() => updatePortfolio(workspace.portfolio.filter((_, itemIndex) => itemIndex !== index))}><Trash2 />{text.remove}</button>
          </div>
        </div>
      </article>)}
      <button className="beauty-workspace-add-button" type="button" onClick={() => updatePortfolio([...workspace.portfolio, createBeautyPortfolioItem(workspace.portfolio.length)])}><Plus />{text.addWork}</button>
    </div>}
    ...
    {tab === "prices" && <div className="beauty-workspace-price-editor">
      ...
      {workspace.services.map((service, index) => <article key={service.id}>
        <header>
          <label className="beauty-workspace-active-toggle"><input type="checkbox" checked={service.active} onChange={(event) => updateService(index, { active: event.target.checked })} /><span>{text.active}</span></label>
          <div className="beauty-workspace-row-actions">
            <button type="button" disabled={index === 0} onClick={() => updateServices(move(workspace.services, index, -1))}><ArrowUp /></button>
            <button type="button" disabled={index === workspace.services.length - 1} onClick={() => updateServices(move(workspace.services, index, 1))}><ArrowDown /></button>
            <button type="button" className="danger" disabled={workspace.services.length <= 1} title={workspace.services.length <= 1 ? text.cannotRemove : text.remove} onClick={() => removeService(index)}><Trash2 />{text.remove}</button>
          </div>
        </header>
        <div className="beauty-workspace-service-grid">
          <label className="is-wide"><span>{text.serviceName} · {languageNames[contentLanguage]}<b>{text.required}</b></span><input value={service.nameByLanguage[contentLanguage]} onChange={(event) => {
            const nameByLanguage = { ...service.nameByLanguage, [contentLanguage]: event.target.value };
            updateService(index, { nameByLanguage, name: resolveBeautyLocalizedText(nameByLanguage, language, "") });
          }} /></label>
