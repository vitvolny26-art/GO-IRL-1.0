...
export function BeautyWorkspaceContentEditor({ workspace, language, onChange }: { workspace: BeautyWorkspace; language: Language; onChange: (next: BeautyWorkspace) => void }) {
  ...
  const updatePortfolio = (portfolio: BeautyWorkspace["portfolio"]) => onChange({ ...workspace, portfolio: portfolio.map((item, index) => ({ ...item, sortOrder: index })) });

  const uploadPhoto = async (index: number, file?: File) => {
    if (!file) return;
    const item = workspace.portfolio[index];
    setUploadingId(item.id);
    setUploadError("");
    try {
      const imageUrl = await uploadBeautyPortfolioPhoto(file);
      updatePortfolio(workspace.portfolio.map((work, itemIndex) => itemIndex === index ? { ...work, imageUrl } : work));
    } catch {
    ...
    {tab === "portfolio" && <div className="beauty-workspace-portfolio-editor">
      ...
      {workspace.portfolio.map((item, index) => <article key={item.id}>
        ...
        <div>
          ...
          <label><span>{text.imageUrl}<i>{text.optional}</i></span><input type="url" inputMode="url" value={item.imageUrl} onChange={(event) => updatePortfolio(workspace.portfolio.map((work, itemIndex) => itemIndex === index ? { ...work, imageUrl: event.target.value } : work))} /></label>
