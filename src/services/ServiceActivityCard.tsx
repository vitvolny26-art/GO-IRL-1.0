...
import { Bell, BellRing, CalendarDays, CalendarPlus, Check, ChevronLeft, ChevronRight, Clock3, Info, MapPin, Ticket, UserRound, UsersRound, X } from "lucide-react";
...
const text = {
  ...
  uk: { details: "Докладніше", book: "Записатися", duration: "Тривалість", price: "Ціна", address: "Адреса", date: "Дата", master: "Майстер", close: "Закрити", booking: "Запис до майстра", chooseDate: "Оберіть дату", chooseTime: "Оберіть час", send: "Надіслати запит", sent: "Запит надіслано", reminder: "Нагадати", reminderTitle: "Нагадування про запис", save: "Зберегти нагадування", slots: "Вільні вікна", noSlots: "Немає вільного часу", previousMonth: "Попередній місяць", nextMonth: "Наступний місяць" },
  ...
  en: { details: "Details", book: "Book", duration: "Duration", price: "Price", address: "Address", date: "Date", master: "Professional", close: "Close", booking: "Book a professional", chooseDate: "Choose a date", chooseTime: "Choose a time", send: "Send request", sent: "Request sent", reminder: "Remind me", reminderTitle: "Booking reminder", save: "Save reminder", slots: "Available slots", noSlots: "No available time", previousMonth: "Previous month", nextMonth: "Next month" },
...
const freeSlotsFor = (date: string, profileId: string) => {
  ...
  const occupied = new Set([
    ...
    ...bookings.filter((item) => item.profileId === profileId && item.date === date && ["pending", "confirmed"].includes(item.status)).map((item) => item.time),
  ]);
...
export function ServiceActivityCard({ professional, language }: { professional: ServicesProfessional; language: Language }) {
  ...
  const cardSlots = useMemo(() => freeSlotsFor(cardDate, professional.profileId), [cardDate, professional.profileId, bookingSent]);
  const bookingSlots = useMemo(() => freeSlotsFor(bookingDate, professional.profileId), [bookingDate, professional.profileId, bookingSent]);
  const [time, setTime] = useState(() => freeSlotsFor(cardDate, professional.profileId)[0] || "");
  ...
  const booking = bookingOpen ? createPortal(<div className="service-sheet-backdrop" onMouseDown={() => setBookingOpen(false)}>
    <section className="service-booking-sheet service-booking-calendar-sheet" role="dialog" aria-modal="true" onMouseDown={(event) => event.stopPropagation()}>
      ...
      ><span>{parseDateKey(date).getDate()}</span><small>{freeSlotsFor(date, professional.profileId).length || "—"}</small></button> : <span key={`empty-${index}`} />)}</div>
      ...
      {bookingSent ? <div className="service-booking-success"><Check />{labels.sent}</div> : <button className="service-sheet-book" type="button" onClick={submitBooking} disabled={!time}><CalendarPlus />{labels.send}</button>}
    </section>
  </div>, document.body) : null;
  ...
  return <>
    <article className="services-professional-card service-activity-card">
      ...
      <div className="service-card-right-stack">
        ...
        {slotsOpen && <div className="service-free-slots-list" role="list">{cardSlots.length ? cardSlots.map((slot) => <button type="button" key={slot} onClick={() => { setSlotsOpen(false); openBooking(cardDate, slot); }}>{slot}</button>) : <span>{labels.noSlots}</span>}</div>}
